...
describe("Sparsko ICO", function () {
  ...
  describe("ICO", function () {
    ...
    it("Should purchase token at each stage of the ICO", async function () {
      ...
      // deploy ICO contract
      const sparksoICO = await SparksoICO.deploy(
      ...
      await expect(
        testToken.transfer(
          sparksoICO.address,
          ethers.utils.parseEther(ICO_SUPPLY.toString())
        )
      ).to.emit(testToken, "Transfer");
      const sparksoICOBalance = await testToken.balanceOf(sparksoICO.address);
      expect(sparksoICOBalance).to.equal(
        ethers.utils.parseEther(ICO_SUPPLY.toString())
      );
      expect(await sparksoICO.getWithdrawableAmount()).to.equal(
        ethers.utils.parseEther(ICO_SUPPLY.toString())
      );
      ...
      await expect(
        sparksoICO.connect(beneficiary).buyTokens(beneficiary.address, {
      ...
      var firstTokens = ethers.utils.parseEther(
        (
          0.8 * calcTokens(ethers.utils.parseEther("1000"), RATE[0], 0)
        ).toString()
      );
      var tokens = ethers.utils.parseEther(
        (
          0.2 * calcTokens(ethers.utils.parseEther("1000"), RATE[0], 0)
        ).toString()
      );
      ...
      ).to.be.equal(firstTokens);
      expect(
        await sparksoICO.computeReleasableAmount(vestingScheduleId2)
      ).to.be.equal(tokens);
      ...
      await expect(
        sparksoICO
          ...
          .releaseTokens(
            ...
            beneficiary.address,
            openingTime,
            buildSignature(beneficiary.address, 0, openingTime)
          )
      ).to.be.revertedWith(
      ...
      await expect(
        sparksoICO
          ...
          .releaseTokens(
            ...
            beneficiary.address,
            deadline,
            buildSignature(beneficiary2.address, 1, deadline)
          )
      ...
      await expect(
        sparksoICO
          ...
          .releaseTokens(
            ...
            beneficiary.address,
            deadline,
            buildSignature(beneficiary.address, 0, deadline)
          )
      )
        .to.emit(testToken, "Transfer")
      ...
      await expect(
        sparksoICO
          ...
          .releaseTokens(
            ...
            beneficiary.address,
            deadline,
            buildSignature(beneficiary.address, 1, deadline)
          )
      )
        .to.emit(testToken, "Transfer")
      ...
      expect(await testToken.balanceOf(beneficiary.address)).to.equal(
        ethers.utils.parseEther(
          calcTokens(ethers.utils.parseEther("1000"), RATE[0], 0).toString()
        )
      );
      ...
      await expect(
        sparksoICO
          ...
          .releaseTokens(
            ...
            beneficiary2.address,
            deadline,
            buildSignature(beneficiary2.address, 0, deadline)
          )
      )
        .to.emit(testToken, "Transfer")
      ...
      expect(await testToken.balanceOf(beneficiary2.address)).to.equal(
        ethers.utils.parseEther(
          calcTokens(
            ethers.utils.parseEther("444019"),
            RATE[0],
            BONUS[0]
          ).toString()
        )
      ...
      await expect(
        sparksoICO
          ...
          .releaseTokens(
            ...
            ethers.utils.parseEther(
              calcTokens(
                ethers.utils.parseEther("1668818.1"),
                RATE[0],
                0
              ).toString()
            ),
            beneficiary3.address,
            deadline,
            buildSignature(beneficiary3.address, 0, deadline)
          )
      ).to.be.revertedWith(
      ...
      await expect(
        ...
        .to.emit(testToken, "Transfer")
        .withArgs(sparksoICO.address, beneficiary3.address, b3_tokens);
      ...
      await expect(
        sparksoICO
          ...
          .releaseTokens(
            ...
            beneficiary3.address,
            deadline,
            buildSignature(beneficiary3.address, 1, deadline)
          )
      )
        .to.emit(testToken, "Transfer")
        .withArgs(
      ...
      // the number of tokens beneficiary should be able to release
      var b4_tokens = ethers.utils.parseEther(
        (
          0.1 * calcTokens(ethers.utils.parseEther("2670109"), RATE[2], 0)
        ).toString()
      ); //"4221000200000000000000000"
      ...
      await expect(
        sparksoICO
          ...
          .releaseTokens(
            ...
            beneficiary4.address,
            deadline,
            buildSignature(beneficiary4.address, 0, deadline)
          )
      ).to.be.revertedWith(
        "TokenVesting: cannot release tokens, not enough vested tokens"
      );
      ...
      await expect(
        sparksoICO
          ...
          .releaseTokens(
            ...
            beneficiary4.address,
            deadline,
            buildSignature(beneficiary4.address, 1, deadline)
          )
      )
        .to.emit(testToken, "Transfer")
      ...
      let restAmount = await sparksoICO.computeReleasableAmount(
        vestingScheduleId
      );
      ...
      await expect(
        sparksoICO
          ...
          .releaseTokens(
            ...
            beneficiary3.address,
            deadline,
            buildSignature(beneficiary3.address, 2, deadline)
          )
      )
        .to.emit(testToken, "Transfer")
        .withArgs(sparksoICO.address, beneficiary3.address, restAmount);
      ...
      await expect(
        sparksoICO
          ...
          .releaseTokens(
            ...
            beneficiary5.address,
            deadline,
            buildSignature(beneficiary5.address, 0, deadline)
          )
      )
        .to.emit(testToken, "Transfer")
      ...
      await expect(
        sparksoICO
          ...
          .releaseTokens(
            ...
            beneficiary5.address,
            deadline,
            buildSignature(beneficiary5.address, 1, deadline)
          )
      )
        .to.emit(testToken, "Transfer")
    ...
    it("Should check delay functionality", async function () {
      ...
      // purchase tokens
      await expect(
        sparksoICO.connect(beneficiary2).buyTokens(beneficiary2.address, {
      ...
      // the number of tokens beneficiary should be able to release
      b1_tokens = ethers.utils.parseEther(
        parseInt(
          calcTokens(ethers.utils.parseEther("4000"), RATE[0], BONUS[0])
        ).toString()
      );
